...
<html>
  ...
  <body style="background-color: white">
    ...
    <div class="row">
        <div class="col-6">
            <div class="text-left">
                <h6 style="font-size: 13px;">REFERENCE #: {{ $application->reference_no }}</h6>
                <h6 style="font-size: 13px;">APPLICANT NAME: {{ $application->lastname }}, {{ $application->firstname }} {{ $application->middlename }}</h6>
                <h6 style="font-size: 13px;">AMOUNT OF: {{ $application->visa_price }}</h6>
                <h6 style="font-size: 13px;">VISA TYPE: {{ $application->visa_type }}</h6>
            </div>
        </div>
        <div class="col-6 offset-6">
            <div class="text-left">
                <h6 style="font-size: 13px;">IN CHARGE: {{ $application->payment_received_by }}</h6>
                <h6 style="font-size: 13px;">DATE OF PAYMENT: {{ date('m/d/Y', strtotime($application->payment_date)) }}</h6>
            </div>
        </div>
    </div>
    <div class="row">
    ...
    <div class="row">
        <div class="col-6">
            <div class="text-left">
                @for ($i = 0; $i < round(COUNT($docs) / 2); $i++)
                    <h6 style="font-size: 13px;">{{ $i + 1 .". " }} {{ $docs[$i]->name }}</h6>
                @endfor
            </div>
        </div>
        <div class="col-6 offset-6">
            <div class="text-left">
                @for ($i = round(COUNT($docs) / 2); $i < COUNT($docs); $i++)
                    <h6 style="font-size: 13px;">{{ $i + 1 .". " }} {{ $docs[$i]->name }}</h6>
                @endfor
            </div>
        </div>
    ...
    <div class="row">
        <div class="col-12">
            <h6 style="font-size: 15px;margin-top: 10%" class="mb-3">TERMS AND CONDITIONS</h6>
